...
class DatasetExplorer:
    def __init__(self, dataset_folder, categories=None, coco_json_path=None):
        ...
        self.image_names = os.listdir(
            os.path.join(self.dataset_folder, "images"))
        ...
        self.categories = [category["name"]
                           for category in self.coco_json["categories"]]
        ...
        if self.coco_json["annotations"]:
            self.last_img_id = self.coco_json["annotations"][-1].get(
                "image_id")
        ...
        self.async_nums = 0
        self.backup_json_path = os.path.join(
            os.path.dirname(self.coco_json_path), "backup.json")
    ...
    def get_annotations(self, image_id, return_colors=False):
        if image_id not in self.annotations_by_image_id:
            return [], []
        cats = [a["category_id"]
                for a in self.annotations_by_image_id[image_id]]
    ...
    def delet_annotation(self, image_id, category_id):
        # 加个判断，避免直接开始就点撤销,或已经为空时的空字典的“KeyError”错误
        # if self.annotations_by_image_id[image_id]:
        # 获取当前图片的标注信息
        current_img_annotations = self.annotations_by_image_id.get(image_id, None)
        if current_img_annotations is None:
            return

        # 找到当前图片选中的类别的标注信息
        current_category_annotations = [ann for ann in current_img_annotations if ann["category_id"] == category_id]
        if not current_category_annotations:
            return
        ...
        del_annotation = current_category_annotations.pop(-1)

        # 要去删除更新 self.annotations_by_image_id 中这个撤销的数据，画图用到了这个变量
        index = self.annotations_by_image_id[image_id].index(del_annotation)
        self.annotations_by_image_id[image_id].pop(index)

        # 再去删除更新要保存的json文件
        index = self.coco_json["annotations"].index(del_annotation)
